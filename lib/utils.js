...
module.exports = {
  ...
  // limit it to bare minimum ascii characters
  asciiOnly: function(text) {
    return text.replace(/[^a-zA-Z0-9_\-\.]/g, '');
  },

  // owner-only names
  ownerOnly: function(text) {
    // do it again. just to be sure
    var resanitized = text.trim();
    resanitized = resanitized.toLowerCase();
    resanitized = resanitized.replace(/0/g, "o");
    return resanitized.startsWith("konk");
  },

  // TODO: disallow some paths and emojis server-side
  validCountry: function(country) {
    return country;
  },
    ...
    if (process.env.DEPLOYMENT == "env") {
      return {
        ...
        slack: {
          ...
          tokens: (process.env.SLACK_TOKENS ? process.env.SLACK_TOKENS.split(",") : [])
        }
      };
    } else
